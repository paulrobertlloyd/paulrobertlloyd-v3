(function() {
    // We'll need the getBoundingClientRect function for the hash scripting, so
    // if it's not supported just return, then the elements will remain in their
    // default open state
    if(typeof(document.createElement('span').getBoundingClientRect) == 'undefined') { return; }

    // Add event construct for modern browsers or IE which fires the callback
    // with a pre-converted target reference
    ...
    // Create a started flag so we can prevent the initialisation function
    // firing from both DOMContentLoaded and window.onloiad
    var started = false;
    ...
    function addDetailsOmnifill(list) {
        ...
        // Get the collection of details elements, but if that's empty then we
        // don't need to bother with the rest of the scripting
        ...
        for(var n = list.length, i = 0; i < n; i ++) {
            ...
            // If the content doesn't have an ID, assign it one now which we'll
            // need for the summary's aria-controls assignment
            if(!details.__content.id) {
                details.__content.id = 'details-content-' + i;
            }

            // Then define aria-controls on the summary to point to that ID
            // so that assistive technologies know it controls the
            // aria-expanded state
            ...
            // Then if this is not a native implementation, create a marker
            // inside the summary, saving its reference as a summary property
            if(!details.__native) {
                var marker = document.createElement('span');
                marker.className = 'details-marker';
                marker.appendChild(document.createTextNode('\u25ba'));

                details.__summary.__marker = details.__summary.insertBefore(marker, details.__summary.firstChild);
            }
        }

        // Define a statechange function that updates aria-expanded and
        // style.display to either expand or collapse the region (ie. invert
        // the current state) or to set a specific state if the expanded flag
        // is strictly true or false then update the marker if we have one with
        // a correpsonding glyph
        function statechange(summary, expanded) {
            ...
            if(summary.__marker) {
                summary.__marker.firstChild.nodeValue = (expanded ? '\u25ba' : '\u25bc');
            }
        ...
        // to pass-through the event, else call and return the statechange
        // function which also returns true to pass-through the remaining event
